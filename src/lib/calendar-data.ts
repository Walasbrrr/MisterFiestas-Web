...
[3, 8, 15, 22, 28].forEach((d) => {
  const key = dateKey(nextYear, nextMonth, d);
  UNAVAILABLE_DATES[key] = ["09:00", "12:00", "13:00", "18:00", "19:00", "20:00"];
});
...
export const EVENT_TYPES = [
  "Cumpleaños", "Boda", "Quinceañera", "Baby Shower",
  "Evento empresarial", "Graduación", "Reunión familiar", "Otro",
];
...
export function isDatePartiallyBooked(year: number, month: number, day: number) {
  const key = dateKey(year, month, day);
